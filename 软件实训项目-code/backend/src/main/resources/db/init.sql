...
CREATE TABLE IF NOT EXISTS `career_profile` (
    ...
    `skill_tags` TEXT DEFAULT NULL COMMENT '技能标签(JSON数组)', -- V3: 从JSON改为TEXT，解决JDBC驱动JSON↔String映射问题
    `target_roles` TEXT DEFAULT NULL COMMENT '目标岗位(JSON数组)', -- V3: 从JSON改为TEXT，解决JDBC驱动JSON↔String映射问题
...
-- V3 迁移：将 JSON 列改为 TEXT，解决 MyBatis-Plus JSON↔String 映射兼容问题
ALTER TABLE `career_profile` MODIFY COLUMN `skill_tags` TEXT DEFAULT NULL COMMENT '技能标签(JSON数组)';
ALTER TABLE `career_profile` MODIFY COLUMN `target_roles` TEXT DEFAULT NULL COMMENT '目标岗位(JSON数组)';
...
CREATE TABLE IF NOT EXISTS `resume_file` (
    ...
    `document_type` VARCHAR(20) DEFAULT NULL COMMENT '文档分类(RESUME/PROJECT/OTHER)',
    `classify_source` VARCHAR(20) DEFAULT NULL COMMENT '分类来源(AI/KEYWORD/UNKNOWN)',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
    KEY `idx_user_id` (`user_id`)
